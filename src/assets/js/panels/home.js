...
class Home {
    ...
    async initNews() {
        ...
        if (this.news) {
                    ...
                    blockNews.innerHTML = `
                        ...
                        <div class="news-content">
                            <div class="bbWrapper">
                                ...
                                <p class="news-author">ne,<span> ${News.author}</span></p>
                            </div>
                        </div>`
                    news.appendChild(blockNews);
                }
            ...
            blockNews.innerHTML = `
                ...
                <div class="news-content">
                    <div class="bbWrapper">
                        <p>sunucuyu baglanamadim .</br>Yapılandırmanızı kontrol edin.</p>
                    </div>
                </div>`
            // news.appendChild(blockNews);
        }
    ...
    async initLaunch() {
        document.querySelector('.play-btn').addEventListener('click', async () => {
            ...
            launch.on('progress', (progress, size) => {
                ...
                document.querySelector(".text-download").innerHTML = `İndiriliyor... ${((progress / size) * 100).toFixed(0)}%`
            ...
            launch.on('check', (progress, size) => {
                progressBar.style.display = "block"
                document.querySelector(".text-download").innerHTML = `Onaylanıyor ${((progress / size) * 100).toFixed(0)}%`
                progressBar.value = progress;
                progressBar.max = size;
            });
    ...
    async initStatusServer() {
        ...
        if (!serverPing.error) {
            ...
            serverMs.innerHTML = `<span class="green">Çevrimiçi</span> - ${serverPing.ms}ms`;
            online.classList.toggle("off");
            playersConnected.textContent = serverPing.playersConnect;
        } else if (serverPing.error) {
            nameServer.textContent = 'oh shit';
